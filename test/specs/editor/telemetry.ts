...
describe('Editor telemetry', () => {
  ...
  test('Telemetry is sent when enabled', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/api/dragify/telemetry/collect');
  ...
  test('Telemetry cleans up old version keys', async () => {
    const sessionStorageMock = {
      ...
      setItem: jest.fn(),
      removeItem: jest.fn(),
      'dragify_telemetry_sent_0.9.0': 'true',
      'dragify_telemetry_sent_0.9.1': 'true',
      other_key: 'true',
    };
    Object.defineProperty(window, 'sessionStorage', { value: sessionStorageMock });
    ...
    expect(sessionStorageMock.setItem).toHaveBeenCalledWith(`dragify_telemetry_sent_${version}`, 'true');
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('dragify_telemetry_sent_0.9.0');
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('dragify_telemetry_sent_0.9.1');
